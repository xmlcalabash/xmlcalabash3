...
val dep_asciidoctorj = project.findProperty("asciidoctorj").toString()
val dep_asciidoctorjPdf = project.findProperty("asciidoctorjPdf").toString()

dependencies {
  implementation(project(":xmlcalabash"))
  implementation("org.asciidoctor:asciidoctorj:${dep_asciidoctorj}")
  implementation("org.asciidoctor:asciidoctorj-pdf:${dep_asciidoctorjPdf}")
...
tasks.register("stage-release") {
  ...
  doLast {
    copy {
      ...
      include("README.md")
      filter { line ->
        line.replace("Asciidoctor extension step",
                     "Asciidoctor extension step version ${xmlbuild.version.get()}")
...
publishing {
  ...
  publications {
    create<MavenPublication>("mavenAsciidoctor") {
      pom {
        groupId = project.findProperty("xmlcalabashGroup").toString()
        version = project.findProperty("xmlcalabashVersion").toString()
        name = "XML Calabash Asciidoctor Step"
        packaging = "jar"
        description = "An Asciidoctor step for XML Calabash 3.x"
...
signing {
  sign(publishing.publications["mavenAsciidoctor"])
}
