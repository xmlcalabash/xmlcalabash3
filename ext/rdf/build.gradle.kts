...
val dep_jena = project.findProperty("jena").toString()
val dep_semargl = project.findProperty("semargl").toString()

dependencies {
  implementation(project(":xmlcalabash"))
  implementation("javax.activation:activation:1.1.1") // For mimetype mapping
  implementation("org.apache.jena:jena:${dep_jena}")
  implementation("org.apache.jena:jena-arq:${dep_jena}")
  implementation("org.apache.jena:jena-core:${dep_jena}")
  implementation("org.apache.jena:jena-iri:${dep_jena}")
  implementation("org.semarglproject:semargl-core:${dep_semargl}")
  implementation("org.semarglproject:semargl-rdfa:${dep_semargl}")
...
tasks.register("stage-release") {
  ...
  doLast {
    copy {
      ...
      include("README.md")
      filter { line ->
        line.replace("RDF extension steps",
                     "RDF extension steps version ${xmlbuild.version.get()}")
...
publishing {
  ...
  publications {
    create<MavenPublication>("mavenRdf") {
      pom {
        groupId = project.findProperty("xmlcalabashGroup").toString()
        version = project.findProperty("xmlcalabashVersion").toString()
        name = "XML Calabash RDF steps"
        packaging = "jar"
        description = "RDF extension steps for XML Calabash 3.x"
...
signing {
  sign(publishing.publications["mavenRdf"])
}
