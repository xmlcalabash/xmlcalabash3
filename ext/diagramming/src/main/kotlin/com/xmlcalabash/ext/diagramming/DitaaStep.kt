...
class DitaaStep(): AbstractAtomicStep() {
    ...
    override fun run() {
        ...
        try {
            ...
            options.processingOptions.characterEncoding = "UTF-8"
            options.renderingOptions.setDropShadows(boolOption(_shadows))
            options.processingOptions.setPerformSeparationOfCommonEdges(boolOption(_separation))
            options.renderingOptions.setAntialias(boolOption(_antialias))

            if (_corners in parameters) {
                val value = parameters[_corners]!!.underlyingValue.stringValue
                if (value == "rounded" || value == "square") {
                    options.processingOptions.setAllCornersAreRound(value == "rounded")
                } else {
            ...
            if (_scale in parameters) {
                ...
                try {
                    options.renderingOptions.scale = value.toFloat()
            ...
            val grid = TextGrid()
            if (options.processingOptions.customShapes != null) {
                grid.addToMarkupTags(options.processingOptions.customShapes.keys)
            }
            ...
            val image = BitmapRenderer().renderToImage(diagram, options.renderingOptions)
