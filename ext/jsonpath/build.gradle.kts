...
val dep_jsonPath = project.findProperty("jsonPath").toString()
val dep_jsonSmart = project.findProperty("jsonSmart").toString()

dependencies {
  implementation(project(":xmlcalabash"))
  implementation("com.jayway.jsonpath:json-path:${dep_jsonPath}")
  implementation("net.minidev:json-smart:${dep_jsonSmart}")
...
publishing {
  ...
  publications {
    create<MavenPublication>("mavenJsonPath") {
      pom {
...
signing {
  sign(publishing.publications["mavenJsonPath"])
}
