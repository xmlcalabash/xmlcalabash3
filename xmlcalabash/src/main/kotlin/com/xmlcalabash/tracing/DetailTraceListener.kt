...
class DetailTraceListener(environment: XProcEnvironment, val path: Path): StandardTraceListener() {
    val savedDocuments = mutableMapOf<Long, String>()

    private val extensionMap = mutableMapOf<MediaType, String>()
    init {
        for ((ext, ctype) in environment.contentTypes) {
    ...
    override fun documentSummary(config: XProcStepConfiguration, builder: SaxonTreeBuilder, detail: DocumentDetail) {
        ...
        builder.addEndElement()
        builder.addStartElement(NsTrace.location, config.typeUtils.attributeMap(mapOf(
