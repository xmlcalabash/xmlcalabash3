...
class CxMetadataExtractorTest {
    ...
    fun testEncryptedPDF() {
        val result = runPipeline(File("src/test/resources/cx-metadata-extractor/pipe-encrypted-pdf.xpl").toURI())
        Assertions.assertTrue(result.value.toString().contains("xmp:ModifyDate"))
    }

    @Test
    fun testPDF() {
        val result = runPipeline(File("src/test/resources/cx-metadata-extractor/pipe.xpl").toURI(), "envelope.pdf")
    ...
    @Test
    fun testBMP() {
        val result = runPipeline(File("src/test/resources/cx-metadata-extractor/pipe.xpl").toURI(), "amaryllis.bmp")
        val compiler = result.context.newXPathCompiler()
    ...
    @Test
    fun testEPS() {
        val result = runPipeline(File("src/test/resources/cx-metadata-extractor/pipe.xpl").toURI(), "amaryllis.eps")
        val compiler = result.context.newXPathCompiler()
        compiler.declareNamespace("c", "http://www.w3.org/ns/xproc-step")
    ...
    @Test
    fun testJPEG() {
        val result = runPipeline(File("src/test/resources/cx-metadata-extractor/pipe.xpl").toURI(), "amaryllis.jpg")
        val compiler = result.context.newXPathCompiler()
        compiler.declareNamespace("c", "http://www.w3.org/ns/xproc-step")
        val selector = compiler.compile("/ * /c:tag[@name='Image Width']").load()
        selector.contextItem = result.value as XdmNode
    ...
    @Test
    fun testImagePDF() {
        val result = runPipeline(File("src/test/resources/cx-metadata-extractor/pipe.xpl").toURI(), "amaryllis.pdf")
        val compiler = result.context.newXPathCompiler()
        compiler.declareNamespace("c", "http://www.w3.org/ns/xproc-step")
    ...
    @Test
    fun testPNG() {
        val result = runPipeline(File("src/test/resources/cx-metadata-extractor/pipe.xpl").toURI(), "amaryllis.png")
        val compiler = result.context.newXPathCompiler()
        compiler.declareNamespace("c", "http://www.w3.org/ns/xproc-step")
        val selector = compiler.compile("/ * /c:tag[@name='Image Width']").load()
        selector.contextItem = result.value as XdmNode
